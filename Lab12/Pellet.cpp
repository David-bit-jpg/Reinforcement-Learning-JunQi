...
void Pellet::OnUpdate(float deltaTime)
{
	...
	{
		GetGame()->GetPlayer()->GetHealthComponent()->TakeDamage(DAMAGE, GetPosition());
		GetGame()->GetAudio()->PlaySound("PelletDeath.ogg", false, this, false);
	...
	for (Actor* c : GetGame()->GetColliders())
	{
			...
			mCollisionComponent->Intersect(c->GetComponent<CollisionComponent>()))
		{
			...
			if (hc)
			{
				...
				{
					hc->TakeDamage(DAMAGE, GetPosition());
				}
			}
