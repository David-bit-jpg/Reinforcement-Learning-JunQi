...
void TurretHead::OnUpdate(float deltaTime)
{
	...
	if (mOldState != mTurretState)
	{
		if (audio->GetSoundState(mSoundHandle) == SoundState::Playing)
		{
			audio->StopSound(mSoundHandle);
		}
		mSoundHandle = audio->PlaySound(mSoundMap[mTurretState], false, this);
	}
	mOldState = mTurretState;
...
void TurretHead::UpdateFiring(float deltaTime)
{
			 ...
			 !mLaserComponent->GetLastHitActor()->GetComponent<HealthComponent>()->IsDead())
	{
		...
		{
			mFireCoolDown = FIRE_INT;
			GetGame()->GetAudio()->PlaySound("Bullet.ogg", false, mAquired);
...
void TurretHead::TakeDamage()
{
	if (!mIsPlayed)
	{
		...
		mSoundHandle = audio->PlaySound("TurretFriendlyFire.ogg", false, this);
		mIsPlayed = true;
	}
}