...
: MoveComponent(actor)
{
	...
	mFootstep = GetGame()->GetAudio()->PlaySound("FootstepLoop.ogg", true);
	GetGame()->GetAudio()->PauseSound(mFootstep);
}

PlayerMove::~PlayerMove()
{
	GetGame()->GetAudio()->StopSound(mFootstep);
}
...
void PlayerMove::Update(float deltaTime)
{
	...
	{
		if (GetGame()->GetAudio()->GetSoundState(mDeathSound) == SoundState::Stopped)
		{
			GetGame()->ReloadLevel();
		}
		else
	...
	if (mCurrentState == MoveState::OnGround && mVelocity.Length() > WALKSOUND_THR)
	{
		GetGame()->GetAudio()->ResumeSound(mFootstep);
	}
	else
	{
		GetGame()->GetAudio()->PauseSound(mFootstep);
...
void PlayerMove::Taunt()
{
	int ind = Random::GetIntRange(0, 3);
	mDeathSound = GetGame()->GetAudio()->PlaySound(mSounds[ind]);
	GetGame()->GetPlayer()->GetHUD()->ShowSubtitle(mSubtitles[ind]);
}