...
class Player:
    ...
    def reflect(self):
        ...
        formulation_prompt = (
            ...
            f"The Leader is also a Player : For example, Leader 1 means Player 1."
            f"If the Leader with the same number as yours, {self.player_id}, this means you proposed this team."
            ...
            f"Here's what you said to the other players {self.released_statement}"
    ...
    def debate(self, proposed_team, is_leader=False, previous_statements=[]):
        ...
        base_prompt = (
            f"You are Player {self.player_id} with role {self.role}. {'You are the Leader, ' if is_leader else ''}"
            f"If the Leader with the same number as yours, {self.player_id}, this means you proposed this team."
            f"Here's your memory for reference {self.filter_memory()}. "
            f"Here's your memory of past quests {self.quest_history_memory}. Consider team choice and quest results, infer the potential role of each player."
            f"If it's the first quest, which means your memory of past quests is none, judge the team selection based on that you observed and try to give more suggestion. (this is a way to show your trust to other players)."
        ...
        refined_debate_prompt = (
            ...
            f"Be brave to suspect other players to gain trust, and rebuttal the suspecion around you to build more trust around the people."
            ...
            f"Finalize your response in one paragraph with fluency like a real, active and enthusiastic human player."
            f"Respond in one paragraph with tone like a real, active human player!"
        )
        refined_debate_response = self.generate_response(refined_debate_prompt)
        self.released_statement.append(refined_debate_response)
    ...
    def get_reflection_messages(self):
        ...
        combined_prompt = "Answer the following questions based on the game context and your memory:\n"
        for item in relevant_messages:
            question = item["question"]
            memory = item["top_k_messages"]
            combined_prompt += f"\nQuestion: {question}\nMemory: {memory}\n"

        combined_prompt += "\nProvide your answers in a clear and concise manner for each question."
